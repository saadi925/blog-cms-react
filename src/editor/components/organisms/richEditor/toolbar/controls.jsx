...
const Controls = (props) => {
  ...
  const renderControlGroups = (controls, id) => {
    const results = controls.map((controlName, index) => {
      ...
      if (availableControls[controlName]) {
        return controlRenderFunctions[availableControls[controlName].method](
          controlName,
          index
        );
      }
      ...
      if (React.isValidElement(customControls[controlName])) {
        return React.cloneElement(customControls[controlName], {
          key: controlName,
        });
      }
      if (customControls && customControls[controlName]) {
        return customListDropdown(
          controlName,
          props.customControls[controlName]
        );
      }
      return null;
    });
  ...
  const customListDropdown = (controlName, controlData) => {
    ...
    return (
      <Dropdown
        render={(props) => <ListContent {...props} />}
        {...customListProps}
      />
    );
  };
  ...
  const controlRenderFunctions = {
    listDropdown: (controlName) => {
      ...
      if (controlName === "blockType") {
        current =
          props.blockType === "pasted-list-item"
            ? "ordered-list-item"
            : props.blockType;
      }
    ...
    formDropdown: (controlName) => {
      ...
      if (controlName === "insertImage") {
        ...
        return (
          <Dropdown
            open={props.openLinkForm}
            render={(props) => <LinkForm {...props} />}
            {...controlProps}
          />
        );
      }
    ...
    controlButton: (controlName) => {
      let active = props.currentStyle
        ? props.currentStyle.has(controlName.toUpperCase())
        : null;
      ...
      if (controlName === "bulletList" || controlName === "numberList") {
        const blockType =
          controlName === "bulletList"
            ? "unordered-list-item"
            : ["ordered-list-item", "pasted-list-item"];
        active =
          blockType === props.blockType ||
          (controlName === "numberList" && blockType.includes(props.blockType));
      }
      const btnProps = {
        active,
        key: controlName,
        ...
        tooltip: Translator.translate(
          availableControls[controlName].tooltip || ""
        ),
